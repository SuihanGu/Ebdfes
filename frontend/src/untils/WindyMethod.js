export const Windy = (params) => {
  ...
  var OPACITY = 0.97;
  var defaulColorScale = ["#00ffff", "#64f0ff", "#87e1ff", "#a0d0ff", "#b5c0ff", "#c6adff", "#d49bff", "#e185ff", "#ec6dff", "#ff1edb"];
  ...
  var start = function start(bounds, width, height, extent) {
    var mapBounds = {
      south: deg2rad(extent[1][1]),
      north: deg2rad(extent[1][0]),
      east: deg2rad(extent[0][0]),
      west: deg2rad(extent[0][1]),
      width: width,
      height: height
    };
    stop(); // build grid
