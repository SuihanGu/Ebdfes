...
The required Python packages are:
    requests, json, datetime, pandas, global_land_mask, 
    math, pymysql, numpy, apscheduler.schedulers.blocking,
    dbutils.pooled_db, threading, warnings, and Estimation, 
    which are essential for web interaction, data processing, 
    scheduling, database management, and concurrent programming.
    
Running `Main.py` directly will initiate the continuous operation 
of the program. Note thatif the correct target database address is
not defined, running `Main.py` directly may result in an error.
 For reproducible examples, refer to `Example_Estimation.py` 
...
if __name__ == '__main__': 
    ...
    station_info_path = os.path.join(BASE_DIR, "station_data.csv")
    ...
    stimu_times=500  # The user can modify it as needed.
    nation_list=['Japan','Japan region','Japan Earthquake'] #The user can modify it as needed.
    url="https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/4.5_day.geojson"
    min_mag=7.0 #The user can modify it as needed.
    try:
        pool = PooledDB(
        ...
        sched.add_job(eq_query,'interval',minutes=5,max_instances=1,
